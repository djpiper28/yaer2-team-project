...
/**
 * Allows user to change the status of an order
 *
 * @author Flynn, Danny
 * @version 1
 */
public class OrderStatusServlet extends HTTPServletOverride {
    /**
     * Logger
     *
     * @since 1
    ...
    public void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            final OrderStatus status = OrderStatus.from(Math.toIntExact((long) object.get("new-status")));
            ...
            order.updateStatus(status, r.getDatabaseConnector());
